...
def write_to_csv(finger_name, finger_angle2, deflection_x, deflection_y):
    ...
    # Extract the base name (without extension) from the finger_name
    file_name = os.path.splitext(os.path.basename(finger_name))[0]

    current_date = date.today().strftime("%Y-%m-%d")
    file_path = os.path.join(base_path, f'results_{file_name}.csv')
    ...
    with open(file_path, mode) as file:
        ...
        # For example:
        file.write(f"{file_name},{finger_angle2},{deflection_x},{deflection_y}\n")

    print('saved')
# Import images
finger_names = glob.glob(r'D:/Nextcloud/00_Promotion/Projekt_Gummifinger/00_Bilder/04_Finger_Teststand/*.jpg')
...
for finger_name in finger_names:
    ...
    plt.close('all')
    # Print a separator for better readability between images
    print('-' * 50)